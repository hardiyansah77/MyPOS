...
/*	public function formAddDormitoryTransaction()
	...
	{
		//$maxjumlahbarang 	= $this->model->selectdt('jumlah','barang')->result_array();
		 //print_r($maxjumlahbarang) ; exit ;
		//$maxjumlahbarang = 2 ;
		/*
		print_r($_POST['tanggal_pinjam']); exit;
		$a = date()
		*/
	...
	public function simpan_transaksi()
        ...
        $this->model->kembalikan($this->input->post('kode_barang'));
...
public function cari_nik()
	{
		$nik = $this->input->post('nik');
		$pencarian = $this->model->SearchNik($nik);
		$data = array (
						'pencarian' => $pencarian,
					);
    ...
    {
        $id_transaksi = $this->input->get_post('id_transaksi');
        // $no_transaksi = 20180411002;
        $hasil = $this->model->SearchTransaksi($id_transaksi);
        if($hasil->num_rows() > 0) {
	...
	public function tampil_barang()
    {
        ...
        $data['barang'] = $this->model->showBarang($id_transaksi)->result();
